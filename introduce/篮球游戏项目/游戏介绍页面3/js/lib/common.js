...
function getPagePoint ( e ) {
    //逻辑或短路
    var x = e.pageX || getPageScroll().scrollLeft + e.clientX;
    var y = e.pageY || getPageScroll().scrollTop + e.clientY;
    return {
        pageX:x,
        pageY:y
    };
...
function addEvent (obj,type,fn ) {
    if (obj.addEventListener){//谷歌火狐
        obj.addEventListener(type,fn, false);
    }else if (obj.attachEvent){//IE8
        obj.attachEvent('on' + type,fn);
    }else{//某些浏览器：两种方式都不支持，用点语法注册
        obj['on' + type] = fn;
    };
}
...
function removeEvent ( obj,type,fn ) {
    if (obj.removeEventListener){//谷歌火狐
